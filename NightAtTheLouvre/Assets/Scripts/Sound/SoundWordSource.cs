...
public class SoundWordSource : MonoBehaviour
{
    ...
    void SpawnSoundWord(Texture2D texture, Vector3 position, float intensity=1f, float lifetime=1f)
    {
        ...
        if (distance > LevelManager.Instance.Player.MaxHearDistance) return;

        Vector3 finalPosition = ClampPositionToScreen(position, playerPosition, mainCamera);
        float distancePercent = 1f - Mathf.Clamp01(distance / LevelManager.Instance.Player.MaxHearDistance);
    ...
    public void PlaySound(int index = 0)
    {
        ...
        SpawnSoundWord(textures[index], transform.position, intensity);
    }
